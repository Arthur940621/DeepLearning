...
class UpsampleBlock(nn.Module):
    def __init__(self, in_c, scale_factor):
        ...
        # (N,C*scale_factor^2,H,W)=>(N,C,H*scale_factor,W*scale_factor)
...
class Discriminator(nn.Module):
    def __init__(self, in_channels=3, features=[64, 64, 128, 128, 256, 256, 512, 512]):
        ...
        for idx, feature in enumerate(features):
            blocks.append(
                ConvBlock(in_channels, feature, kernel_size=3, stride=1+idx%2, padding=1, discriminator=True, use_act=True, use_bn=False if idx == 0 else True)
            )
            in_channels = feature
